...
object UserVisitSessionAnalyzeSpark23 extends Serializable {

  //val master = "local"
  val master = "yarn-cluster"
  val appName = "UserVisitSessionAnalyzeSpark23"
  ...
  def getSessionInfo(path: String): Dataset[Row] = {
    spark.read.parquet(path)
  }

  def getProductInfo(path: String): Dataset[Row] = {
    spark.read.parquet(path)
  }

  def getUserInfo(path: String): Dataset[Row] = {
    spark.read.parquet(path)
  }
  ...
  def filterSessionAndAggrStat(sessionid2AggrInfoRDD: RDD[(String, String)], taskParam: TaskParam,sessionAggrStatAccumulator: AccumulatorV2[String, String]) = {
    val startAge = 0
    val endAge = 100
  ...
  /**
    ...
    */
  def randomExtractSession(taskid: Int, spark: SparkSession, sessionid2AggrInfoRDD: RDD[(String, String)], sessionid2ActionRDD: RDD[(String, Row)]): Unit = { //得到<yyyy-MM-dd_HH, aggrInfo>
  ...
  /**
    ...
    * @param sessionid2DetailRDD
    */
  def getTop10Session(spark: SparkSession, taskId: Int, top10CategoryList: List[Tuple2[CategorySortKey, String]], sessionid2DetailRDD: RDD[(String, Row)]): Unit = {
    /**
